...
Suppose further that we know a set of $F$ functional annotations or properties for each of our $G$ genes.  Let $\textbf{A}$ denote the $F$ by $G$ matrix of annotations such that $A_{f,g}$ is the value of annotation $f$ for gene $g$.
We define the vector $\boldsymbol{\beta}$ and the function $\pi(\boldsymbol{\beta},\textbf{A_{.,g}})$ such that:

$$P\pi(\boldsymbol{\beta},\textbf{A}_{.,g}) =  \frac{1}{1+e^{-(\beta_{0}+\sum_{f=1}^F{A_{f,g}\beta_f})}} =  (Z_g=1|\textbf{A}_{.,g},\boldsymbol{\beta})$$
...
$$P(Z_g=1 | x_g,\boldsymbol{\beta},A_{.,g}) = \frac{\pi(\boldsymbol{\beta},\textbf{A}_{.,g}) B_g}{\pi(\boldsymbol{\beta} , \textbf{A}_{.,g}) B_g + 1-\pi(\boldsymbol{\beta},\textbf{A}_{.,g})}} $$
