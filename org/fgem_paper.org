...
Our fitting procedure consists of minimizing the objective function corresponding to the negative of the penalized log-likelihood:
...
takes on 75 values of  between zero, and $\lambda_{\text{max}}$, on a log scale.  $\lambda_{\text{max}}$ is defined as the smallest value of $\lambda$ for which the objective function is minimized when all coefficients but the intercept are 0 (the intercept is not subject to the elastic net penalty).
The final value of $\lambda$ was arrived at from 10-fold cross-validation.  
