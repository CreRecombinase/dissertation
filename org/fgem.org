...
#+BEGIN_SRC R :session fg :colnames yes :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ :colnames yes
  library(progressr)
  library(future.apply)
  library(fgem)
  plan(multisession)
  cancerf <- fs::path_ext_remove(fs::path_file(fs::dir_ls("data/fdr_models/")))
  fgem_results_df <- readRDS("data/fgem_data.RDS")
  od <- "data/new_correct_elasticnet_models"
  fs::dir_create(od)
#+END_SRC
...
| x                              |
|--------------------------------|
| data/correct_elasticnet_models |

#+BEGIN_SRC R :session drivermaps :colnames yes :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ :colnames yes
    iseq <- seq_along(cancerf)
      with_progress({
        p <- progressor(along = iseq)  
        ret_df <- future.apply::future_lapply(iseq,
                                    function(tf,cancerf,od){
                                      cancer <- cancerf[tf]
                                      p(sprintf("i=%g", tf))
                                      ...
                                      if(!file.exists(op)){
                                        ...
                                        trdf <- dplyr::mutate(fgem::fgem_bfgs(X, ym, alpha=0.8),
                                                              cancer=cancer)
                                        qs::qsave(trdf, op)
                                        return(trdf)
                                      }else{
...
#+BEGIN_SRC R :session drivermaps :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
  ...
  pp %>% pwalk(function(cancer,Beta,post,...){
    ...
    od <- fs::path("org/fgem_post_plots")
    fs::dir_create(od)
    ggsave(plt,filename=fs::path(od,cancer,ext="png"))
  })
#+END_SRC
