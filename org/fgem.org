...
#+BEGIN_SRC R  :session tada :dir /home/nwknoblauch/Dropbox/Repos/FGEM/
  ...
  colnames(ibf_df)[-1]
  cn <- colnames(ibf_df)[-1][1]
  #for(cn in colnames(ibf_df)[-1]){
    tdf <- ibf_df[,c("gene",cn)] %>% magrittr::set_colnames(c("gene","BF"))
    ixl_df <- inner_join(tdf,feature_df) %>% rename(Gene=gene)
    tp_df <- select(ixl_df,Gene,BF,pLI) %>% fgem::FGEM_df(verbose=TRUE)
  #}
