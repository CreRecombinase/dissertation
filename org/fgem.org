...
#+BEGIN_SRC R :session fisher :dir /home/nwknoblauch/Dropbox/Repos/dissertation/
  ...
  qs::qsave(fisher_df,"data/clean_single_fisher.qs")
#+END_SRC
...
cancer_df <- qs::qread("data/clean_driverMAPS_results_20TumorTypes.qs")
    ...
    msig_df <- group_by(msig_df, feature_name) %>%
      ...
      select(feature_name) %>%
      inner_join(msig_df)
    msig_df  <-  mutate(msig_df,value=1.0)
    ...
    nidf <-   nest(min_cancer_df,cdf=c("Gene","BF")) %>% mutate(ix=1:n())
  ...
  with_progress({
    ...
    pwalk(nidf,function(cancer,cdf,ix){
      p(sprintf("x=%g",ix))
      X <- as.matrix(fgem::trip2sparseMatrix(rowname_vals = as.character(msig_df$Gene),
...
#+BEGIN_SRC R :session univ  
  fs::dir_create("data/msigdb_results/")
  ix  <- 1
  library(fgem)
  library(progressr)
  marg_fun <- function(ix,cancerf){
    cancer <- cancerf[ix]
    X <- qs::qread(fs::path("data/msigdb_X",cancer,ext="qs"))
    cdf <- qs::qread(fs::path("data/msigdb_y",cancer,ext="qs"))

    fgmr <- dplyr::mutate(fgem:::fgem_marginal(X,cdf$BF,parallel=TRUE),cancer=cancer)
    p(sprintf("x=%g",ix))
    qs::qsave(fgmr,fs::path("data/msigdb_results/",cancer,ext="qs"))
    return(cancer)
  }
...
#+BEGIN_SRC R :session jacc :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
  ...
  fisher_df <- qs::qread("data/clean_single_fisher.qs") %>%
      dplyr::select(cancer,
...
#+BEGIN_SRC R :session fitmult :dir /ssh:trcc:/home/nwknoblauch/xm
    ...
    out_d <- fs::path("data/cv_elasticnet_models/")
    fs::dir_create(out_d)
...
#+BEGIN_SRC R :session fg :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
  ...
  long_df <- qs::qread("data/msigdb.qs") %>% select(-feature_url) %>% unnest(Gene)
#  long_df <- qs::qread("data/GO_df.qs")
  blong_df <- bind_rows(long_df,
...
#+BEGIN_SRC R :session fg :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ :results graphics :file org/univ_multiv.png
  ...
  ## GO_def <- readRDS("data/GO_Definitions.RDS") %>%
  ##   dplyr::select(feature_name,Term)
  ...
  fit_df <- mutate(fit_df,cv_max=max(cv_sum),lik0=cv_sum[group_l1==0],lambda_max=group_l1[cv_sum==cv_max])
  feat_l  <- unnest(fit_df,Beta) %>% filter(feature_name!="Intercept") %>% distinct(cancer,feature_name) %>% nest(feat_list=-cancer)
...
#+BEGIN_SRC R :session fg :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ :results graphics :file org/univ_multiv.png
  ...
  top_fits <- filter(fit_df,lambda_max>0, group_l1>=lambda_max) %>%
    inner_join(cancer_dfl) %>%
    mutate(data=map2(Beta, data,add_pred),
           Beta=map(Beta,inner_join,y=select(MS_def,-Gene,-feature_url),by="feature_name")) %>%
    ungroup()

  refit_fun <- function(cancer,group_l1,group_l2,Beta,feat_list,data,...){
    lambda <- group_l1/0.85
    alpha <- 0.85
    fldf <- feat_list
    fldf <- add_row(fldf,feature_name="Intercept",.before = 1)
    beta_df <- left_join(fldf,Beta) %>% replace_na(list(Beta=0))
    stopifnot(beta_df$feature_name[1]=="Intercept")
    X <- as.matrix(xGO[data$Gene,beta_df$feature_name[-1],drop=FALSE])
    fgr <- fgem:::fgem_elasticnet(X,data$BF,Beta0=beta_df$Beta,alpha=alpha,lambda=lambda)
    dplyr::mutate(fgr,cancer=cancer,data=list(add_pred(Beta[[1]],data)))
  }
...
#+BEGIN_SRC R :session fg :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ :results graphics :file org/univ_multiv.png
  ...
  assign_intogen <-  function(data,cancer){
    intogen_l <- mutate(sub_comp_df,intogen_gene = if_else(cancer == {{cancer}},
  ...
  plot_post <- function(data){
    kt <- c("Known Type-Specific Cancer Gene (Non-TCGA)",
            ...
            "Unknown Cancer Gene")
    data <-   filter(data,
                     uniform_posterior>1e-05,
                     functional_posterior>1e-05
                     ) %>% arrange(desc(functional_posterior-uniform_posterior)) %>%
      mutate(idn=1:n(),
             `intOGen Validation Status`=factor(kt[intogen_gene],levels=kt))
    top_diff <- filter(data,idn < 15 | as.integer(`intOGen Validation Status`)<3 | functional_posterior > 0.95 | uniform_posterior > 0.95)
    ...
    data %>%
      ggplot(aes(x=uniform_posterior,
                    ...
                    label=Gene)) + 
       geom_point() +
       geom_label_repel(data=top_diff,max.overlaps=50) +
  ...
  plot_beta <- function(Beta){
    ...
    Beta_p <- tBeta %>% arrange(abs(Beta)) %>%
      mutate(label=str_wrap(paste0(feature_name,":\n",description),width=50),
             ...
             idx=1:n())
    top_Beta_p <- filter(Beta_p,Beta_ct==Beta_ct[l])
    ggplot(top_Beta_p,aes(y=feature_name,x=Beta,label=description)) +
      geom_point(col="red") + ylab("Feature index")+
      geom_text_repel(max.overlaps=50,direction="x")+theme_minimal()
  }


  plt_fun <- function(Beta,data,cancer,l0n,...){
     ## out_png_e <- fs::path("org/t_cv_fits_enrichment")
     ## fs::dir_create(out_png_e)
     out_png_p <- fs::path("org/l_cv_fits_posterior")
     fs::dir_create(out_png_p)

     out_png_p <- fs::path(out_png_p,cancer,ext="png")
     ## out_png_e <- fs::path(out_png_e,cancer,ext="png")
     ## pa <- plot_beta(Beta) + theme_fira() + plot_annotation(title=cancer,subtitle=paste0("L0: ",as.integer(l0n)))
     ## ggsave(out_png_e,pa,dpi=320,width=13.33,height=7.5)
     pb <- plot_post(data) + theme_fira()+ plot_annotation(title=cancer,subtitle=paste0("L0: ",as.integer(l0n)))
     ggsave(out_png_p,pb,dpi=320,width=13.33,height=7.5)

     ## plt <- pa/pb & theme_minimal()
     ## ggsave(out_png,nplt,dpi=320,width=13.33,height=7.5)
  }
  ...
  top_ofit_genes <- new_best_fits %>% mutate(top_diff = map(data, 
                                                            function(data){
                                                              arrange(data,
                                                                      desc(functional_posterior - uniform_posterior)) %>%
                                                                slice(1:5) 
                                                         })) %>%
     select(cancer, top_diff) %>%
     unnest(top_diff)

   top_ofit_feature_gene <- select(new_best_fits, cancer, Beta) %>%
     unnest(Beta) %>%
     inner_join(inner_join(top_ofit_genes,long_df))
  top_ofit_features <- top_ofit_feature_gene %>%
    distinct(cancer, feature_name, Beta) %>%
    inner_join(GO_def)

   semi_join(ungroup(cancer_df),filter(long_df,feature_name=="GO:0050911")) %>% arrange(desc(BF)) %>% pull(Gene) %>% head(10) %>% unique() %>% paste0(collapse=",")
