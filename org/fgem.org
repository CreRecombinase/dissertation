...
* TODO Fisher's exact test detailed comparison
** TODO create table for fisher's exact test
...
**** TODO table of full results
***** TODO each feature and beta, univariate p value and beta
***** TODO each gene BF functional and uniform prior (and pathways) and evidence of the gene
*** TODO Introduction 
...
*** TODO number of (significant) genes per cancer
*** TODO find out what the outliers are
...
#+BEGIN_SRC R :session bioc :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ :results none
  ...
  ## setCache(directory = "/run/media/nwknoblauch/c67d20d8-a8bf-4ab9-91af-978fa311014a/cbio")
  ## cbio <- cBioPortal()
...
You can download the full msigdb from https://www.gsea-msigdb.org/gsea/msigdb/download_file.jsp?filePath=/msigdb/release/7.1/msigdb.v7.1.symbols.gmt (you have to register)

#+BEGIN_SRC R :dir /home/nwknoblauch/Dropbox/Repos/dissertation/
  library(tidyverse)
  msigf <- "data/msigdb.v7.1.symbols.gmt"
  #msig_xml <- read_xml("~/Dropbox/Repos/dissertation/data/msigdb_v7.1.xml") %>% xml_root() %>% xml_children() %>% map_dfr(~tibble(!!!xml_attrs(.x))) 
  msig_xml <- qs::qread("data/msig_metadata.qs")


  msig_db <- read_lines(msigf) %>%
    str_split(pattern="\t") %>% map_dfr(function(x){
      tibble(feature_name=x[1],feature_url=x[2],Gene=list(x[-c(1,2)]))
      })
  msig_xml <- qs::qread("data/msig_metadata.qs")
  
  msig_meta <- msig_xml %>% select(feature_name=STANDARD_NAME,
                                   organism=ORGANISM,
  ...
  qs::qsave(msig_db,"data/msigdb.qs")
  qs::qsave(msig_meta,"data/msigdb_metadata.qs")
...
#+BEGIN_SRC R :session fisher :dir /home/nwknoblauch/Dropbox/Repos/dissertation/
  ...
  cancer_df <- qs::qread("data/clean_driverMAPS_results_20TumorTypes.qs") 
...
return(bind_rows(ret_2,ret_1,ret_05,ret_01))
  ...
  qs::qsave(fisher_df,"data/clean_single_fisher_cutoffs.qs")
#+END_SRC
...
#+BEGIN_SRC R :session univ :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
    msig_df <- qs::qread("data/msigdb.qs") %>% select(-feature_url) %>% unnest(Gene)  %>% semi_join(distinct(cancer_df,Gene))
    msig_meta <- qs::qread("data/msigdb_metadata.qs")
    #25701
    msig_df <- group_by(msig_df, feature_name) %>%
      summarise(n_gt = n()) %>%
      filter(n_gt >= 10) %>%
      select(feature_name) %>%
      inner_join(msig_df)
    msig_df  <-  mutate(msig_df,value=1.0) %>% semi_join(filter(msig_meta,str_detect(sub_category,"Legacy",negate=T)),by="feature_name")
    # 20837
#+END_SRC
...
#+BEGIN_SRC R :session univ :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
  ...
  epsilon <- 1e-06
  max_iter <- 150
  fs::dir_create("data/msigdb_X")
  fs::dir_create("data/msigdb_y")
  with_progress({
    p <- progressor(along = seq_len(nrow(nidf)))
    pwalk(nidf,function(cancer,cdf,ix){
      p(sprintf("x=%g",ix))
      X <- fgem::trip2sparseMatrix(rowname_vals = as.character(msig_df$Gene),
                                             colname_vals = msig_df$feature_name,
                                             values = msig_df$value,
                                             total_rownames = cdf$Gene,
                                             total_colnames = unique(msig_df$feature_name),
                                             add_intercept = FALSE,
                                             csr=FALSE)
      oxf <- fs::path("data/msigdb_X",cancer,ext="qs")
      oyf <- fs::path("data/msigdb_y",cancer,ext="qs")
      qs::qsave(X,oxf)
      qs::qsave(cdf,oyf)
    })
  })
...
#+BEGIN_SRC R :session univ  
    fs::dir_create("data/fgem_m_results/")
    fs::dir_create("data/fgem_m_results_OG/")
    fs::dir_create("data/fgem_m_results_TSG/")

    library(fgem)
    library(progressr)
  plan(list(multisession))
    marg_fun <- function(ix,cancerf){
      cancer <- cancerf[ix]
      cdf <- qs::qread(fs::path("data/msigdb_y",cancer,ext="qs"))
      X <- qs::qread(fs::path("data/x_models",cancer,ext="qs"))
      X <- X[cdf$Gene,colnames(X)!="Intercept",drop=FALSE]
    ...
    ixv <- seq_along(cancerf)
    with_progress({
      p <- progressor(along = ixv)
      resl <- furrr::future_map(ixv,marg_fun,cancerf=cancerf)
    })
    ...
    ## }
    ## Qworkers <- workers(n_jobs=5,data=msig_df,verbose=TRUE,template=list(memory=8096,cores=4),log_worker=TRUE)
    ## qres <- Q_rows(nidf,marg_fun,pkgs=c("future","fgem"),workers=Qworkers)
#+END_SRC
...
#+BEGIN_SRC R :session drivermaps :dir /home/nwknoblauch/Dropbox/Repos/dissertation/
  ...
  with_progress({
    p <- progressor(along = seq_len(nrow(nidf)))
    nidf %>% furrr::future_pmap_dfr(function(cancer,cdf,ix){
      p(sprintf("x=%g",ix))
      ...
      BF <- cdf$BF
      fgmr <- fgem_marginal(X,BF)
      mutate(fgmr,cancer=cancer)
    }) %>% 
    qs::qsave("data/alt_new_fgem_marginal_results.qs")
  })
...
#+BEGIN_SRC R :session jacc  :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
  ...
  library(purrr)
  library(Matrix)
  msig_df <- map_df(fs::dir_ls("data/msigdb_results"),qs::qread) %>% 
  select(Beta,time,convergence,pval,qval,sum_X,cancer) 
  msig_xml <- qs::qread("data/msig_metadata.qs")
  ...
  msig_meta <- msig_xml %>% select(feature_name=STANDARD_NAME,
                                   ...
                                   sub_category=SUB_CATEGORY_CODE,
                                   description=DESCRIPTION_BRIEF,
                                   Genes=MEMBERS_SYMBOLIZED)

  msig_df <- msig_df %>% mutate(feature_name=map_chr(Beta, pluck, "feature_name", 2))
  msm_sum <- group_by(msig_meta,category,sub_category) %>% summarise(feat_ct=n())

  feat_df <- inner_join(msig_df,msig_meta)
  group_p  <- filter(feat_df,convergence==0) %>%
    group_by(cancer,category) %>%
    mutate(group_bq=p.adjust(pval)) %>% ungroup()


  arrange(feat_df,qval) %>%
    filter(qval<0.05) %>%
    select(-Beta,-Genes) %>%
    count(cancer,category,sub_category) %>%
    arrange(desc(n)) %>%
    inner_join(msm_sum) %>%
    mutate(label=paste(category,sub_category,sep=":"),perc_sig=n/feat_ct) %>%
    ggplot(aes(x=cancer,y=perc_sig,fill=cancer))+geom_bar(stat="identity")+facet_wrap(~label)+theme(axis.text.x=element_text(angle=90))
#+END_SRC
...
#+BEGIN_SRC R :session jacc :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
  ...
  msig_df <- map_df(fs::dir_ls("data/msigdb_results"),qs::qread)
   results_df <- qs::qread("data/clean_new_fgem_marginal_results.qs")  %>%
     group_by(cancer) %>%
     filter(iter<100) %>%
   ...
   sig_features <- group_by(results_df,cancer) %>%
     filter(qval<0.2)  %>%
     ungroup() %>%
     select(cancer,feature=feature_name,qval)
   ...
   map(function(x){
     tdf <- unnest(x,gene_l)
     as.matrix(trip2sparseMatrix(as.character(tdf$gene_l),tdf$feature,total_rownames=levels(tdf$gene_l),add_intercept=FALSE))
   })
   x_mats <- x_mats[order(map_int(x_mats,NCOL))]
   yl <- split(cancer_df$BF,cancer_df$cancer)
   yl <- yl[names(x_mats)]

   ox <- fs::path("data/new_models_hdf")
   fs::dir_create(ox)
   ...
   ody <- fs::path("data/clean_y_models")
   fs::dir_create(ody)
   odx <- fs::path("data/clean_x_models")
...
#+BEGIN_SRC R :session jacc :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
  ...
  fgem_results_df <- fgem_results %>%
    mutate(trait = map_chr(Beta, ~.x$feature_name[2])) %>% unnest(Beta) %>% 
  ...
  fisher_df <- qs::qread("data/clean_single_fisher_cutoffs.qs") %>%
      dplyr::select(cancer,
                    feature_name,
                    pval = p.value,cutoff) %>%
...
#+BEGIN_SRC R :session univ  :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
    ...
    library(future)
    library(Matrix)
  if(!file.exists("~/tmp/feat_df.qs")){

    msig_df <- map_df(fs::dir_ls("data/msigdb_results"),qs::qread) %>% select(Beta,time,convergence,pval,qval,sum_X,cancer) 
    msig_xml <- qs::qread("data/msig_metadata.qs")
    ...
    feat_df <- inner_join(msig_df,msig_meta)
    qs::qsave(feat_df,"~/tmp/feat_df.qs")
  }else{
  feat_df <- qs::qread("~/tmp/feat_df.qs")
  }
    sig_feat_df <- filter(feat_df,qval<0.1)
    ssig_feat <- filter(feat_df,qval<0.01)
#+END_SRC

#+RESULTS:

#+BEGIN_SRC R :session tuniv  :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 

    cancer <- "SARC"

    cancerf <- unique(feat_df$cancer)
    cancerf <- cancerf[cancerf!="CHOL"]
  out_d <- fs::path("data/cv_msigdb_models_2/")
  fs::dir_create(out_d)
   cancerf <-  cancerf[! cancerf %in% fs::path_ext_remove(fs::path_file(fs::dir_ls(out_d)))]


  ssig_feat <- filter(feat_df,qval<0.1)
    rl <- furrr::future_map(cancerf, function(cancer){
      cdf <- filter(ssig_feat,cancer=={{cancer}})
      cf <- fs::path("data/multi_msigdb_X/",cancer,ext="qs")
      sX <- qs::qread(cf)
      ydf <- qs::qread(fs::path("data/msigdb_y",cancer,ext="qs"))
      out_d <- fs::path("data/cv_msigdb_models_2/")
        tfg <- fgem::cv_fgem(sX, ydf$BF, alpha = 0.95)
      qs::qsave(mutate(tfg,cancer={{cancer}}),fs::path(out_d,cancer,ext="qs"))
      return(tfg)
    })
...
#+BEGIN_SRC R :session fitmult :dir /ssh:trcc:/home/nwknoblauch/xm
  ...
  out_d <- fs::path("data/cv_gobp_nostrat_models/")
  fs::dir_create(out_d)
...
#+BEGIN_SRC R :session fg :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
  library(tidyverse)
                                          #oo_cv_elasticnet_models/
  ret_df <- map_df(fs::dir_ls("data/o_cv_elasticnet_models/",glob="*qs"),qs::qread)
  new_ret_df <- map_df(fs::dir_ls("data/cv_elasticnet_models/"),qs::qread)

  cret_df <- bind_rows(mutate(new_ret_df,
  cv_i = as.character(cv_i),
  model = "new"),
  mutate(ret_df, model = "old")) 




  ret_df0  <-  filter(cret_df, group_l1 == 0) %>%
    group_by(cancer,model) %>%
    summarise(cv_lik0 = mean(cv_lik),
              cv_sum0 = sum(cv_lik)) %>%
    ungroup()

  cv_df <-  cret_df %>% 
    group_by(cancer, group_l1, group_l2, model) %>%
    summarize(
      cv_sum = sum(cv_lik),
      l0_mean = mean(l0n),
      l0_sd = sd(l0n),
    ...
    ungroup() %>%
    group_by(cancer, model) %>% 
    mutate(rel_lik = c(scale(cv_sum)),
           best_lambda = group_l1[which.max(cv_sum)][1]) %>% 
    ungroup() 
  ...
  cv_df  %>% 
    filter(group_l1 > 0) %>% 
    ggplot(aes(x = group_l1, y = rel_lik, col = model)) +
    geom_point() +
    facet_wrap(~cancer, scale = "free_y") +
    scale_x_log10() +
    geom_vline(aes(xintercept = best_lambda, col = model)) +
    xlab("L1 Penalty") +
    ylab("CV-Likelihood") +
    geom_hline(aes(yintercept = lik0, col = model))



  cv_df  %>% 
    filter(group_l1 > 0) %>% 
    ggplot(aes(x = group_l1, y = l0_mean, col = model)) +
    geom_point() +
    facet_wrap(~cancer, scale = "free_y") +
    scale_x_log10() +
    geom_vline(aes(xintercept = best_lambda, col = model)) +
...
#+BEGIN_SRC R :session fg :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ :results graphics :file org/univ_multiv.png
  GO_def <- readRDS("data/GO_Definitions.RDS") %>%
    select(feature_name,Term)

  fgem_results <- readRDS("data/fgem_data.RDS") %>%
    unnest(data) %>%
    select(cancer,Beta_univariate=Beta,feature_name)
  head(cv_df)
  best_cv <- filter(cv_df, model == "new") %>%
    filter(group_l1 == best_lambda, cv_sum > lik0) %>%
    select(cancer, best_group_l1 = group_l1, best_group_l2 = group_l2)


  unrdf <- unnest(new_ret_df,Beta)
  best_ret_p <- new_ret_df %>%  inner_join(best_cv) %>% unnest(Beta)

  filter(best_ret_p,group_l1==best_group_l1,feature_name!="Intercept",best_group_l1>0) %>%
    group_by(cancer, feature_name, group_l1,best_group_l1) %>%
    summarise(mean_Beta = mean(Beta),
              sd_Beta = sd(Beta)) %>%
    ungroup() %>%
    group_by(cancer) %>%
    filter(abs(mean_Beta)==max(abs(mean_Beta))) %>%
    ungroup()  %>%
    inner_join(GO_def) %>%
    mutate(label=paste0(feature_name,":\n",Term)) %>%
    ggplot(aes(x=mean_Beta,y=label,label=cancer)) + geom_point() + geom_text_repel()
  ...
  ggp <- filter(best_ret_p,group_l1>0,group_l1<=1e-03) %>%
    ...
    ggplot(aes(x = group_l1, y = mean_Beta, col = feature_name)) +
    geom_line()+
    scale_x_log10()+
...
#+BEGIN_SRC R :session fg :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
  ...
  library(ggrepel)
  library(patchwork)
  ocancer_df <- qs::qread("data/clean_driverMAPS_results_20TumorTypes.qs") 
  cancer_df <-  ocancer_df %>%
    mutate(BF = exp(BF)) %>%
    select(Gene,cancer,BF) %>% 
    group_by(cancer) %>%
    mutate(uniform_posterior = c(fgem:::BF2posterior(BF,Gene)))
...
#+BEGIN_SRC R :session fg :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
  ...
  ofit_df <- map_df(fs::dir_ls("data/cv_msigdb_models/"), qs::qread) %>%
    group_by(cancer) %>% mutate(universe="msigdb")
  gofit_df <- map_df(fs::dir_ls("data/cv_elasticnet_models/"), qs::qread) %>%
    group_by(cancer) %>% mutate(universe="gobp")

  fit_df  <- bind_rows(ofit_df,gofit_df)  %>%
    group_by(cancer,universe) %>% 
    fgem:::summarise_cv_lik(.)


  fit_df <- mutate(fit_df,
                   cv_max = max(cv_sum),
                   lik0 = cv_sum[group_l1 == 0],
                   lambda_max = group_l1[cv_sum == cv_max])

  feat_l  <- unnest(fit_df, Beta) %>% 
    filter(feature_name != "Intercept") %>%
    distinct(cancer, feature_name) %>%
    nest(feat_list = -cancer)
#+END_SRC
...
#+BEGIN_SRC R :session fg :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
  ...
  add_pred <- function(beta_df,cancer_df){
      ret_df <- mutate(cancer_df, functional_prior = c(fgem:::predict_long_df(beta_df, xMS[cancer_df$Gene,])),
                       functional_posterior=c(fgem:::BF2posterior(BF, Gene, functional_prior)))
      return(ret_df)
  }
...
#+BEGIN_SRC R :session fg :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ :results graphics :file org/univ_multiv.png
    ...
    assign_intogen <-  function(data,cancer,include_tcga=TRUE){
      ...
      intogen_l <- mutate(scdf,intogen_gene = if_else(cancer == {{cancer}},
        ...
        left_join(data, intogen_l, by="Gene") %>%
    ...
    bidf <- unnest(fit_df,Beta) 
    Beta_mdf <- nest(bidf,bl=c(-cancer,-universe,-lambda_max)) %>% mutate(Beta_m=map(bl,~fgem:::df2sparse(.x,rownames="group_l1",colnames="feature_name",value="Beta",total_rownames=unique(.x[["group_l1"]]))) %>% map(function(mat){
      cm <- colnames(mat)
      t(mat[,c("Intercept",cm[cm!="Intercept"])])
      }))



  cancer_dfl <- nest(cancer_df,data=-cancer) %>% mutate(data=map2(data,cancer,assign_intogen,include_tcga=TRUE))
  Beta_mdf <- inner_join(Beta_mdf,cancer_dfl)
  ##   ivL <- map(cancer_dfl,~as.integer(.x$intogen_gene!=5))
  ##   B_l <- group_mapn(cancer_df,~pull(.x,BF))
  ##   X_l <- map(Beta_m,function(x){xMS[,rownames(x)[-1],drop=FALSE]})
  ##   G_l <- group_mapn(cancer_df,~pull(.x,Gene))
  ## cancer_dfl <- cancer_dfl[names(cancer_dfl)!="CHOL"]
  Beta_mdf <- filter(Beta_mdf,cancer!="CHOL")
  val_l <- Beta_mdf %>%
    pmap(function(cancer,universe,Beta_m,data,bl,...){
    
    
      Beta <- Beta_m
      G <- data$Gene
      stopifnot(all(rownames(Beta)[-1] %in% colnames(xMS)))
      X <- xMS[G,rownames(Beta)[-1]]
      B <- data$BF
      XB <- xb <- t(t(X %*% Beta[-1, ]) + Beta[1, ])
      lB <- log(B)
    
      tp <- fgem:::gen_p(Beta, X)
      tlp <- fgem:::gen_p(Beta, X, log = TRUE)
      lposts <- fgem:::log_BF2_logpost(lB, XB)
      iv_nt <- as.integer(data$intogen_gene%in% c(1,3))
    
      likvec_nt <- apply(lposts,2,function(lp,x){
        sum(x*lp+(1-x)*R_Log1_Exp(lp))
      },x=iv_nt)
    
      iv <- as.integer(data$intogen_gene%in% c(1,2,3,4))
      likvec <- apply(lposts,2,function(lp,x){
        sum(x*lp+(1-x)*R_Log1_Exp(lp))
      },x=iv)
    
      return(bind_rows(
        tibble(lambda=as.numeric(names(likvec)),cross_entropy=as.numeric(likvec),method="tcga"),
        tibble(lambda=as.numeric(names(likvec_nt)),cross_entropy=as.numeric(likvec_nt),method="notcga"),
        ))
    }
    )

  Beta_mdf <- ungroup(Beta_mdf) %>% mutate(cvl=val_l)
  select(Beta_mdf,cancer,universe,lambda_max,cvl) %>% filter(universe=="gobp") %>% unnest(cvl) %>% filter(lambda>0) %>% ggplot(aes(x=lambda,y=cross_entropy,col=method))+geom_point()+facet_wrap(~cancer,scales="free_y")+scale_x_log10()+geom_vline(aes(xintercept=lambda_max))

  intogen_cv_df <- select(Beta_mdf,cancer,universe,cvl,lambda_max) %>% unnest(cvl) %>% 
      group_by(cancer,universe,method) %>%
      mutate(cross_entropy = dplyr::if_else(is.infinite(cross_entropy) | is.na(cross_entropy), -Inf, cross_entropy)) %>% 
      mutate(l1_intogen_max = lambda[cross_entropy == max(cross_entropy)][1]) %>%
      ungroup() %>%
      rename(group_l1=lambda)

  filter(intogen_cv_df,group_l1!=0,universe=="gobp") %>%
    ggplot(aes(x=group_l1,y=cross_entropy,col=method)) +
    geom_point() +
    geom_vline(aes(xintercept=lambda_max),col="gold")+
    facet_wrap(~cancer,scales="free_y") +
    scale_x_log10() +
    geom_vline(aes(xintercept=l1_intogen_max,col=method))


  nfit_df <- fit_df %>% rename(l0=l0_mean) %>% inner_join(intogen_cv_df)
  nfit_df <- filter(nfit_df,l0==min(l0))  %>% summarise(ce_null=max(cross_entropy)) %>% inner_join(nfit_df)
  nfit_df %>%
    filter(group_l1>0,universe=="gobp",cancer!="KICH") %>%
    ggplot(aes(x = l0, y = cross_entropy, color = (cross_entropy > ce_null) & l0 > 1)) +
    geom_point() +
    facet_wrap(~cancer, scales = "free")
...
#+BEGIN_SRC R :session fg :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
    best_fits <- filter(nfit_df,lambda_max>0, group_l1==lambda_max, as.integer(l0) > 1) %>%
      inner_join(nest(cancer_df,data=-cancer)) %>%
      mutate(data=map2(Beta, data,add_pred),
             Beta=map(Beta,inner_join,y=all_def,by="feature_name")) %>%
      inner_join(feat_l) %>%
      dplyr::mutate(data = map2(data, cancer, assign_intogen, include_tcga = TRUE))

  best_fit_relax <- pmap_dfr(best_fits,function(cancer,group_l1,group_l2,Beta,data,...){
    gb <- filter(Beta,Beta!=0,feature_name!="Intercept")
    X <- xMS[data$Gene,gb$feature_name]
    lbf <- log(data$BF)
    alpha <- group_l1/(group_l1+group_l2)
    o_lambda <- group_l1/alpha
    ifg <- fgem:::fgem_elasticnet(X,lbf,alpha=1,lambda=o_lambda,log_BF=TRUE)
    data <- add_pred(ifg$Beta[[1]],data)
    ifg$Beta[[1]] <- left_join(ifg$Beta[[1]],all_def,by="feature_name")
    mutate(ifg,cancer=cancer,data=list(data),l0n=nrow(gb)+1)
  })

    tight_best_fits <-   filter(nfit_df,lambda_max>0,cross_entropy>ce_null, group_l1==l1_intogen_max, as.integer(l0) > 1) %>%
        inner_join(nest(cancer_df,data=-cancer)) %>%
        mutate(data=map2(Beta, data,add_pred),
               Beta=map(Beta,left_join,y=all_def,by="feature_name")) %>%
        inner_join(feat_l) %>%
        dplyr::mutate(data = map2(data, cancer, assign_intogen))
#+END_SRC
...
#+BEGIN_SRC R :session fg :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
  plot_post <- function(data,log=TRUE){
    ...
    top_diff <- filter(data,
                       functional_posterior>1e-05) %>% slice(1:25)
    top_g <- filter(data, idn>=15, functional_posterior > 0.99 | uniform_posterior > 0.99) %>%
      group_by(`intOGen Validation Status`) %>% arrange(desc(pmax(functional_posterior,uniform_posterior))) %>% slice(1:15) %>% ungroup()
    ttg <- filter(data,intogen_gene==5,functional_posterior > 0.9)
    ...
    if(log){
      rp <- rp + scale_x_log10() + scale_y_log10()
    }
    return(rp)
  }
  ...
  plt_fun <- function(Beta, data, cancer, l0n,...){
     out_png_e <- fs::path(paste0("org/relax_","gobp","_enrichment"))
     fs::dir_create(out_png_e)
     out_png_p <- fs::path(paste0("org/relax_","gobp","_posterior"))
...
#+BEGIN_SRC R :session fg :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
  ...
  feat_df <- qs::qread("data/alt_new_fgem_marginal_results.qs")  %>%
    unnest(Beta) %>%
    filter(feature_name != "Intercept") %>%
    select(feature_name,
           cancer,
  ...
  ## rfeature_odf <- unnest(select(best_fit_relax,-data),Beta)  %>%
  ##   filter(Beta != 0) %>% 
  ##   select(cancer, feature_name, Beta, category, sub_category, description) %>%
  ##   inner_join(feat_df) %>%
  ##   arrange(cancer,desc(abs(Beta)))
  
    feature_odf %>% write_tsv("data/all_sig_features.tsv")



  feature_odf <- unnest(select(best_fits,-data),Beta)  %>%
    filter(Beta != 0, universe == "gobp") %>% 
    select(cancer, feature_name, Beta, category, sub_category, description) %>%
    inner_join(feat_df) %>%
    arrange(cancer,desc(abs(Beta)))
  ...
  fit_df <- inner_join(fit_df,group_by(cancer_df,cancer) %>%
                              summarise(n_genes=n())) %>%
    filter(universe=="gobp") %>%
    group_by(cancer) %>% select(-universe) %>% 
    mutate(cv_bic=l0_mean*log(n_genes)-2*cv_sum)

  fit_df <- mutate(fit_df,cv_bic=2*cv_sum-l0_mean*log(n_genes))
  fit_df <- inner_join(select(fit_df,-lambda_bic_max),summarise(fit_df,lambda_bic_max=group_l1[cv_bic==max(cv_bic)]))


  fit_df %>% ggplot(aes(x=group_l1, y=cv_bic)) +
    geom_point() +
    facet_wrap(~cancer,scales="free_y") +
    scale_x_log10() + geom_vline(aes(xintercept=lambda_max))+geom_vline(aes(xintercept=lambda_bic_max),col="green")


  huge_bf <- unnest(select(best_fits,-data),Beta) %>%
    filter(Beta!=0,universe=="gobp") %>% 
    inner_join(blong_df) %>%
    inner_join(unnest(select(filter(best_fits,universe=="gobp"), -Beta), data)) %>%
    mutate(intogen_gene=kt[intogen_gene])

  filter(huge_bf, Beta != 0) %>%
    filter(Beta < 0)

  gene_list_df <- distinct(huge_bf,Gene,cancer,BF,functional_prior,functional_posterior,uniform_posterior,intogen_gene) %>% 
    inner_join(
      filter(huge_bf,Beta!=0) %>% 
    group_by(cancer, Gene,intogen_gene) %>%
    summarise(features=paste0(feature_name,collapse=";"))) %>% arrange(cancer,desc(functional_posterior))


    write_tsv("data/all_sig_genes.tsv")
...
#+BEGIN_SRC R :session gbm :dir /home/nwknoblauch/Dropbox/Repos/dissertation/ 
  ...
  cancer <- "GBM"
  ocancer_df <- qs::qread("data/clean_driverMAPS_results_20TumorTypes.qs")  %>% filter(cancer=={{cancer}})
